...
namespace eiibd26.Controllers
{
    ...
    public class SintomasAdminController : ControllerBase
    {
            ...
            bool sellarProcesada = false)
        {
            ...
            if (sellarProcesada)
                sintoma.RegeneracionProcesadaUtc = DateTime.UtcNow;
        ...
        public async Task<IActionResult> GenerateIaDescription(int id, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (!await PasaGateAsync(sintoma, decision, cancellationToken, sellarProcesada: true))
                {
                    return Ok(new
                    {
                        ok        = false,
                ...
                if (!reconocido)
                {
                    ...
                    await PasaGateAsync(sintoma, deGenerador, cancellationToken, sellarProcesada: true);
                    return Ok(new
                    {
                        ok    = false,
            ...
            try
            {
                ...
                if (request.Regenerar)
                {
                    ...
                    baseQuery = baseQuery
                        ...
                        // nada (el skip del navegador se pierde, la marca en BD no).
                        .Where(s => s.RegeneracionProcesadaUtc == null)
                ...
                // para llenar el ChangeTracker con filas que se van a descartar igual.
                ...
                foreach (var sintomaId in ids)
                {
                    ...
                    try
                    {
                        ...
                        var sintoma = await _db.sintomas
                            .FirstOrDefaultAsync(s => s.id == sintomaId && !s.Eliminado, cancellationToken);
                    ...
                    {
                        fallo = true;
                        _logger.LogError(ex, "Error al procesar síntoma {Id}: {Nombre}", sintomaId, nombreActual);
                    ...
                    finally
                    {
                        ...
                        // registro SIGUIENTE. Caso real en producción — dos síntomas salieron ❌
                        // por fallos de BD y quedaron igual sellados y con descripción nueva, de
                        // arrastre. Eso rompe el retry (se asume "falló ⇒ quedó pendiente") y hace
                        // que `consumidos` subcuente, con lo que el cliente adelanta el cursor de
                        // más y se salta un pendiente por cada fallo.
                    ...
                    if (fallosSeguidos >= FallosSeguidosParaAbortar)
                    {
                        ...
                        _logger.LogError(
                            "Batch de síntomas abortado: {Fallos} fallos seguidos. Procesados {Procesados} de {Total}.",
